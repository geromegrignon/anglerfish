  ...
  return (
    <>
      ...
      <div className="absolute top-8 md:top-4 left-4 right-4 z-20 max-w-3xl mx-auto">
        <div className="h-4 bg-gray-700/80 rounded overflow-hidden backdrop-blur-sm border border-red-500/30">
          <div 
        ...
        <div className="flex justify-between items-center mt-2 px-6">
          ...
          <div className="flex items-center">
            {Array.from({ length: 3 }).map((_, index) => (
              <Heart
                key={index}
                className={`w-6 h-6 ml-1 text-purple-500 ${
                  index < hitPoints ? 'fill-purple-500' : 'fill-none'
                }`}
              />
            ))}
          </div>
